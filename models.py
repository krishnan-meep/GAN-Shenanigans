...
class Generator(nn.Module):
	#Image size should be a tuple, preferably a multiple of 16 
	def __init__(self, image_size, noise_dim = 128, specnorm = False):
		super(Generator, self).__init__()
		self.h, self.w = image_size[0]//16, image_size[1]//16

		self.Proj = nn.Linear(noise_dim, self.h*self.w*1024)
		self.B0 = nn.BatchNorm2d(1024)
		self.MN0 = ModulatingNet(1024, specnorm = specnorm)

		self.TC1 = nn.ConvTranspose2d(1024, 512, kernel_size = 4, stride = 2, padding = 1)
		self.B1 = nn.BatchNorm2d(512)
		self.MN1 = ModulatingNet(512, specnorm = specnorm)

		self.TC2 = nn.ConvTranspose2d(512, 256, kernel_size = 4, stride = 2, padding = 1)
		self.B2 = nn.BatchNorm2d(256)
		self.MN2 = ModulatingNet(256, specnorm = specnorm)

		self.TC3 = nn.ConvTranspose2d(256, 128, kernel_size = 4, stride = 2, padding = 1)
		self.B3 = nn.BatchNorm2d(128)
		self.MN3 = ModulatingNet(128, specnorm = specnorm)

		self.TC4 = nn.ConvTranspose2d(128, 3, kernel_size = 4, stride = 2, padding = 1)
...
class Discriminator(nn.Module):
	def __init__(self, image_size):
		...
		self.h, self.w = image_size[0]//32, image_size[1]//32
		...
		self.DB5 = DownBlock(512, 512, specnorm = True, batchnorm = False)
        
		self.D1 = SpectralNorm(nn.Linear(512*self.h*self.w + 512, 1))
